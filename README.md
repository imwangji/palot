...
### Wangji sub2api integration

This workspace fork is configured to use the OpenAI provider as an
OpenAI-compatible sub2api gateway.

1. Open Settings -> Providers.
2. Connect OpenAI.
3. Enter the `sk-...` API key generated by `https://sub2api.bywangji.com`.
4. Keep the default endpoint:

```text
https://sub2api.bywangji.com/v1
```

Palot still runs OpenCode locally. sub2api is used as the account, billing, and
model gateway layer.
